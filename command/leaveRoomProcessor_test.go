...
func Test_leaveRoomProcessor_Run(t *testing.T) {
	...
	p, _ := NewLeaveRoomProcessor(getBasicProcessor())

	user := entity.NewUser(0, "conn")

	obj := &[]config.LeaveRoomCmdData{
		config.LeaveRoomCmdData{},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("leaveRoomProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
