...
func Test_userInfoProcessor_Run(t *testing.T) {
	...
	p, _ := NewUserInfoProcessor(getBasicProcessor())

	//user for command run
	user := entity.NewUser(999, "conn")

	//user id for get user
	obj := &[]config.UserInfoCmdData{
		{
			UserID: 1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("userInfoProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
