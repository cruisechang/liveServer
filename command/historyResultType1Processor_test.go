...
func Test_resultType1Processor_Run(t *testing.T) {
	...
	room, _ := nx.GetRoomManager().CreateRoom(100, conf.RoomType1(), "room")
	hall.AddRoom(room)

	resultHistory := roomConf.HistoryResultType1{
		0, 1, 2, 0, 1, 2, 0, 1, 2, 1,
		0, 1, 2, 0, 1, 2, 0, 1, 2, 1,
		0, 1, 2, 0, 1, 2, 0, 1, 2, 1,
		0, 1, 2, 0, 1, 2, 0, 1, 2, 1,
	...
	p, _ := NewHistoryResultType1Processor(getBasicProcessorByRCTRL(nx,rCtrl))
	...
	cmd := &nex.CommandObject{
		Cmd: &nex.Command{
			Code:    0,
			Command: conf.CmdHistoryResultType1(),
			Step:    0,
			Data:    cs,
		},
		User: user,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("resultType1Processor.Run() error = %v, wantErr %v, name=%s", err, tt.wantErr, tt.name)
			}
		})
	}
}
