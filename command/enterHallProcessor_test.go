...
func Test_enterHallProcessor_Run(t *testing.T) {
	...
	p, _ := NewEnterHallProcessor(getBasicProcessor())

	user := entity.NewUser(0, "conn")

	obj := &[]config.EnterHallCmdData{
		{
			HallID: 0,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := p.Run(tt.args.obj); (err != nil) != tt.wantErr {
				t.Errorf("enterHallProcessor.Run() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
